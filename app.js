...
// ----------------------------
// Sound option selector (1=internal,2=mp3-1,3=mp3-2)
// ----------------------------
const SOUND_OPTIONS = [
  "internal",
...
let soundOptionIndex =
  isNaN(storedSoundOpt) ||
  storedSoundOpt < 1 ||
  storedSoundOpt > SOUND_OPTIONS.length
    ? 1
    : storedSoundOpt;

function getCurrentSound() {
  return SOUND_OPTIONS[soundOptionIndex - 1];
}

// Position sound button to the right of volume bar with 20px gap; vertically centered with volume bar
let SOUND_BUTTON_RECT; // will be assigned after volume bar rect is defined
...
// Moved 40px closer: 40 (button) + 200 width + 20 gap = 260
const VOLUME_BAR_RECT = { x: 260, y: 530, width: 250, height: 30 };

// Now we can set SOUND_BUTTON_RECT based on VOLUME_BAR_RECT
SOUND_BUTTON_RECT = {
  x: VOLUME_BAR_RECT.x + VOLUME_BAR_RECT.width + 20,
  y: VOLUME_BAR_RECT.y + (VOLUME_BAR_RECT.height - 20) / 2,
  width: 20,
  height: 20,
};
...
class CountdownTimer {
  constructor(label, seconds, rect, sound = "internal") {
    this.label = label;
    this.initial = seconds; // seconds
    this.remaining = seconds;
    this.running = false;
    this.targetEnd = null; // timestamp in seconds when countdown ends
    this.pausedAt = 0; // ms when paused
    this.sound = sound;
    this.rect = rect;
  }
  ...
  start() {
    ...
    this.targetEnd = nowSec + this.remaining;
    this.running = true;
  }
  ...
  tick(now) {
    ...
    this.remaining = Math.max(0, this.targetEnd - nowSec);
    if (this.remaining <= 0) {
      this.remaining = 0;
      this.running = false;
      playAlert(getCurrentSound()); // use global selected sound
...
canvas.addEventListener("click", (e) => {
  ...
  // Sound option button handling
  if (contains(SOUND_BUTTON_RECT, clickX, clickY)) {
    soundOptionIndex = (soundOptionIndex % SOUND_OPTIONS.length) + 1; // cycle 1-3
    localStorage.setItem("soundOptionIndex", soundOptionIndex);
    return;
  }
...
function drawSoundButton() {
  ...
  ctx.fillRect(
    SOUND_BUTTON_RECT.x,
    SOUND_BUTTON_RECT.y,
    SOUND_BUTTON_RECT.width,
    SOUND_BUTTON_RECT.height
  );
  ctx.strokeStyle = currentTheme.stroke;
  ctx.lineWidth = 2;
  ctx.strokeRect(
  ...
  ctx.textBaseline = "middle";
  ctx.fillText(
    String(soundOptionIndex),
    SOUND_BUTTON_RECT.x + SOUND_BUTTON_RECT.width / 2,
    SOUND_BUTTON_RECT.y + SOUND_BUTTON_RECT.height / 2 + 1 // shift down 1px
  );
  ctx.restore();
}
